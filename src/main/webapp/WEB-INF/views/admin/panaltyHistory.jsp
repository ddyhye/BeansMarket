	...
	<main>
		...
		<section>
			<div class="content-search">
				<p>검색</p>
				<div class="search-box" data-value="member_penalty">
					<input type="text" name="search-val" id="subject-search" placeholder="이메일 혹은 제재 번호를 입력해주세요" />
					<button class="search-btn"><i class="fa-solid fa-magnifying-glass"></i></button>
				</div>
			</div>

            <table id="penalty-result-box">
                <thead>
                    <tr>
                        <th>제재 번호</th>
                        <th>제재 옵션</th>
...
function historyDraw(list){
	...
	for (item of list) {
		content +='<tr>';
		content +=		'<td>'+item.penalty_idx+'</td>';
		content +=		'<td>'+item.option+'</td>';
		content +=		'<td>'+item.content+'</td>';
		content +=		'<td>'+DateToString(item.penalty_date)+'</td>';
		content +=		'<td>'+item.email+'</td>';
		content +=		'<td>'+item.admin_name+'</td>';
		content +='</tr>';
	}

	$('#penalty-result').html(content);

	...
	<main>
		...
		<section>
			<div class="content-search">
				<p>검색</p>
				<div class="search-box" data-value="alarm">
					<input type="text" name="search-val" id="subject-search" placeholder="이메일 혹은 알림 번호를 입력해주세요" />
					<button class="search-btn"><i class="fa-solid fa-magnifying-glass"></i></button>
				</div>
			</div>

            <table id="alarm-hist-box">
                <thead>
                    <tr>
                        <th>알림 번호</th>
                        <th>내용</th>
                ...
                <tbody id="alarm-hist-result"></tbody>
            </table>
...
function historyDraw(list){
	...
	for (item of list) {
		...
		content +=		'<td>'+item.link+'</td>';
		content +=		'<td>'+DateToStringTime(item.reg_date)+'</td>';
		content +=		'<td>'+item.email+'</td>';
		content +=		'<td>'+item.checked+'</td>';
		content +='</tr>';
	}

	$('#alarm-hist-result').html(content);

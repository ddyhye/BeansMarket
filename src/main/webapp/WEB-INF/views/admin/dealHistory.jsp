	...
	<main>
		...
		<section>
			<div class="content-search">
				<p>검색</p>
				<div class="search-box" data-value="deal_detail">
					<input type="text" name="search-val" id="subject-search" placeholder="이메일 혹은 번호를 입력해주세요" />
					<button class="search-btn"><i class="fa-solid fa-magnifying-glass"></i></button>
				</div>
			</div>

            <table id="deal-hist-box">
                <thead>
                    <tr>
                        <th>거래 번호</th>
                        <th>글 번호</th>
                ...
                <tbody id="deal-hist-result"></tbody>
            </table>
...
function historyDraw(list){
	...
	for (item of list) {
		content +='<tr>';
		content +=		'<td>'+item.deal_idx+'</td>';
		content +=		'<td>'+item.idx+'</td>';
		content +=		'<td>'+item.seller_email+'</td>';
		content +=		'<td>'+item.buyer_email+'</td>';
		content +=		'<td>'+item.blind+'</td>';
		content +=		'<td>'+DateToStringTime(item.reg_date)+'</td>';
		content +='</tr>';
	}

	$('#deal-hist-result').html(content);
